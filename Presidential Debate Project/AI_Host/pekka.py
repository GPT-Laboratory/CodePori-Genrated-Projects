...
class main:
    def __init__(self,message):
        ...
        self.bot_1 = open("agent_pekka.txt", "r").read()
        #self.bot_2 = open("agent_002.txt", "r").read()
        #self.bot_3 = open("agent_003.txt", "r").read()
        #self.bot_4 = open("agent_004.txt", "r").read()
        
        # self.project_description = get_user_input()
        self.project_description = message
        if not self.project_description:
            exit()

        self.bot_1 = self.bot_1.replace("PROJECT_DESCRIPTION", self.project_description)
        #self.bot_2 = self.bot_2.replace("PROJECT_DESCRIPTION", self.project_description)
        #self.bot_3 = self.bot_3.replace("PROJECT_DESCRIPTION", self.project_description)
        #self.bot_4 = self.bot_4.replace("PROJECT_DESCRIPTION", self.project_description)
